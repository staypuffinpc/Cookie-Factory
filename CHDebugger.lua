...
-- CodeHelper Debugger
-- Copyright 2012 Bogdan Vladu
-- Modified for use with CodeHelper.
-- www.gamedevhelper.com
...
local function start(controller_host, controller_port)
  ...
  if server then
	...
	server:send("Connected to " .. controller_host .. ":" .. controller_port);
...
local function handle(params, client, options)
  ...
  or command == "over" or command == "exit" then
    print("Continuing execution, please wait...");
...
local function listen(host, port)
  ...
  if file and line then
    ...
    print("CodeHelper debugger is ready")
  else
